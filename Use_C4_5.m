...
%         make_tree(train_patterns, train_targets, inc_node, discrete_dim, max(discrete_dim), 0);  
    ...
    if ((inc_node > L) | (L == 1) | (length(Uc) == 1)) %如果剩余训练样本太小(小于inc_node)，或只剩一个，或只剩一类标签，退出  
        H                   = hist(double(targets), double(length(Uc)));  %统计样本的标签，分别属于每个标签的数目    H(1×标签数目)
    ...
    for i = 1:Ni %遍历每个特征  
        ...
        if (discrete_dim(i)) %如果这个特征是离散特征 
            ...
            %Calculate the information for each possible split  计算分裂信息度量
              ...
              for j = 1:Nbins-1
                  P(:, 1) = hist(double(sorted_targets(find(sorted_data <= Ud(j)))) , double(Uc));  %记录<=当前特征值的样本的标签的分布情况
                  P(:, 2) = hist(double(sorted_targets(find(sorted_data > Ud(j)))) , double(Uc));  %记录>当前特征值的样本的标签的分布情况
